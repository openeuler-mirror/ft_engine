...
MapperAdapter::MapperAdapter()
{
    if (GrallocInitialize(&mapperFuncs_) != HDF_SUCCESS) {
        LOG_ERROR("%{public}s: mapperFuncs_ init failed", __func__);
    }
}
...
MapperAdapter::~MapperAdapter()
{
    if (GrallocUninitialize(mapperFuncs_) != HDF_SUCCESS) {
        LOG_ERROR("%{public}s: mapperFuncs_ uninit failed", __func__);
    }
}

bool MapperAdapter::IsReady() const
{
    LOG_INFO("%{public}s: entry", __func__);
